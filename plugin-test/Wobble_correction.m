function varargout = Wobble_correction(varargin)
...
%
%      WOBBLE_CORRECTION('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in WOBBLE_CORRECTION.M with the given input arguments.
%
%      WOBBLE_CORRECTION('Property','Value',...) creates a new WOBBLE_CORRECTION or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before Wobble_correction_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to Wobble_correction_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help Wobble_correction

% Last Modified by GUIDE v2.5 04-Sep-2014 17:53:47

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @Wobble_correction_OpeningFcn, ...
                   'gui_OutputFcn',  @Wobble_correction_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before Wobble_correction is made visible.
function Wobble_correction_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to Wobble_correction (see VARARGIN)

% Choose default command line output for Wobble_correction
...
% UIWAIT makes Wobble_correction wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = Wobble_correction_OutputFcn(hObject, eventdata, handles) 
...
function pushbutton_doCorrectWobble_Callback(hObject, eventdata, handles)
%update the parameters
handles.wobblePath= get(handles.edit_wobblePath,'String');
...
z=evalin('base',handles.handlesPsvGui.settings.varz);

%wobble correct
zScaleFactor=1;%for now don't include z rescale
[xC,yC] = correct3D(x,y,z,handles.wobblePath,zScaleFactor);

%apply correction
assignVars(handles,xC,yC);


function edit_wobblePath_Callback(hObject, eventdata, handles)


function edit_wobblePath_CreateFcn(hObject, eventdata, handles)
...
function pushbutton_selectWobblePath_Callback(hObject, eventdata, handles)
FilterSpec='*.txt';
DialogTitle = 'Select wobble calibration file';
[FileName,PathName] = uigetfile(FilterSpec,DialogTitle); 
if FileName~=0
   fpath = fullfile(PathName,FileName);
   handles.wobblePath = fpath;
   set(handles.edit_wobblePath,'String',handles.wobblePath);
end
...
%%------------------------------------------------------------------------------
%%------------------------------------------------------------------------------
%function [xC,yC] = wobbleCorrect(x,y,z,wobblePath);
%
%fprintf('\nCorrect wobble!\n');
%
%xC=x;
