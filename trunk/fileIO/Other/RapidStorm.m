...
%-----------------------------------------------------------
%-----------------------------------------------------------
function [data semanticCell parInfo]= readRapidStorm(fname)
%read rapidstorm XML
% outputs :
% data : (n,m), n colums of frames, for m rows of parameters
% semanticCell: cell size n, containing the semantic for each column
% parInfo	:
%   .ATTRIBUTE
%     .insequence : true/false
%     .repetitions : "variable"/ ?
%   .field  : nx1 struct array defining each column of SR localizations
%     .ATTRIBUTE:
%        .identifier :
%        .semantic   :
...
%convert the header to an xml structure
xmlStr = srFileStruct.textdata{:};
% rapidStorm header has a '# ' at the start of the header which needs to be removed
xmlStr = regexprep(xmlStr,'^# *','');
%"simple" java code to create a document from a string (from http://stackoverflow.com/questions/9489713/parsing-xml-strings-in-matlab )
xmlHead = javax.xml.parsers.DocumentBuilderFactory.newInstance().newDocumentBuilder.parse(java.io.StringBufferInputStream(xmlStr));
parInfo = xml_read(xmlHead);% using xml_read from xml_io_tools_2010_11_05

data= srFileStruct.data;

nCol = numel(parInfo.field);
